...
const clientRoot = path.resolve(root, "apps", "kodi");
    ...
    return fs
        ...
        .filter((entry) => {
            ...
            return (
                ...
                !relative.startsWith(".test-profile/") &&
...
function packageAddon(): string {
    ...
    const sbom = {
        ...
        metadata: {
            ...
            tools: { components: [{ type: "application", name: "tooling/kodi/release.ts" }] },
        },
        components: [
            {
                type: "framework",
    ...
    const provenance = {
        ...
        source: { repository: "CardGameDigital", path: "apps/kodi" },
        build: {
...
if (command === "package") packageAddon();
    ...
    if (!zipPath) throw new Error("Usage: release.ts verify <artifact.zip>");
    verifyZip(zipPath);
    console.log(path.resolve(zipPath));
} else throw new Error("Usage: release.ts <package|verify>");
