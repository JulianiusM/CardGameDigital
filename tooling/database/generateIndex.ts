...
/**
 * What we generate:
 *   - One file: apps/server/src/modules/database/__index__.ts
...
const CONFIG: SectionConfig[] = [
    {
        key: "entities",
        dir: "packages/persistence/entities",
        exclude: ["index.ts"],
        filePattern: /\.ts$/,
    ...
    {
        key: "subscribers",
        dir: "apps/server/src/modules/database/subscribers",
        exclude: ["index.ts"],
        filePattern: /\.ts$/,
...
const OUTPUT_FILE = path.resolve("apps/server/src/modules/database/__index__.ts");
 ...
 */
function extractExportsByStrategy(
    tsCode: string,
    strategy: SectionConfig["matchStrategy"],
): {
    name: string;
    isDefault: boolean;
}[] {
...
async function generate() {
    ...
    const unformatted = `${header}${allImports.join("\n")}\n\n${sectionExports.join("\n\n")}\n`;
    const prettierConfig = (await resolveConfig(OUTPUT_FILE)) ?? {};
    const output = await format(unformatted, {
        ...prettierConfig,
        filepath: OUTPUT_FILE,
    });
    const existing = readFileOrNull(OUTPUT_FILE);
    if (existing === output) {
        console.log(`\n✅ Generated database index is current`);
        return;
    }
    if (checkOnly) {
        throw new Error("Generated database index is out of date; run npm run generate:db-index");
    }
