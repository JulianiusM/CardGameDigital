...
import type { Locale } from "../../../../packages/localization/messages";
import type { DataSpace } from "../../../../packages/persistence/entities/user/DataSpace";
...
export async function createDataSpace(
    ...
    await persistSession(session);
    // Keep the established additive response fields while declaring the canonical
    // browser projection required by consumers.
    return created;
}
...
export async function accountSnapshot(session: Request["session"]): Promise<AccountSnapshot> {
    const user = await requireUser(session);
    ...
    return {
        ...
        dataSpaces: dataSpaces.map(dataSpaceSummary),
    };
}

function dataSpaceSummary(
    dataSpace: Pick<DataSpace, "id" | "name" | "defaultForOwner">,
): DataSpaceSummary {
    return {
        id: dataSpace.id,
        name: dataSpace.name,
        defaultForOwner: dataSpace.defaultForOwner,
    };
}
