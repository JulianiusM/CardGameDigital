...
from .native_settings_metadata import AUTOMATIC_LOCALE, SOURCE_LOCALE
...
def run() -> None:
    ...
    try:
        ...
        def locale() -> str:
            ...
            if current and current.state.preferences.locale != AUTOMATIC_LOCALE:
                return current.state.preferences.locale
            return xbmc.getLanguage(xbmc.ISO_639_1, region=True) or SOURCE_LOCALE
        ...
        if not monitor.abortRequested():
            xbmcgui.Dialog().notification(
                ...
                addon.getLocalizedString(bootstrap_strings.ADDON_START_FAILED),
