...
from .. import strings as bootstrap_strings
from ..contract_data import generated_design_tokens
...
from ..protocol.validation import NO_VOTE, YES_VOTE
INTERACTIVE_LISTS = (
    ROW_LIST,
    CARD_ACTION_LIST,
    CHOICE_LIST,
...
_DESIGN_COLORS = generated_design_tokens()["colors"]
DANGER_FOCUS_TINT = f"FF{str(_DESIGN_COLORS['raspberry']).lstrip('#')}".upper()
...
class MainWindow(xbmcgui.WindowXMLDialog):
    ...
    def _replace_items(self, control_id: int, items: tuple) -> None:
        ...
        for current in items:
            ...
            row.setProperty("FocusTint", DANGER_FOCUS_TINT if current.danger else "FFFFFFFF")
            rows.append(row)
        if rows:
    ...
    def _render_loading(self) -> None:
        ...
        self.setProperty("Heading", self.addon.getLocalizedString(bootstrap_strings.LOADING))
        for name in (
            "Body",
            "Alert",
            "ServerPill",
        ...
        self.setProperty("BackLabel", self.addon.getLocalizedString(bootstrap_strings.BACK))
        self.setProperty("HelpLabel", self.addon.getLocalizedString(bootstrap_strings.HELP))
