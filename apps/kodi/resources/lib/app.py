...
def _named_result_needs_auto_page(
    ...
    if voting.get("revealMode") != NAMED_REVEAL_MODE or not isinstance(named, list):
        return False
    yes_count = sum(1 for entry in named if entry.get("vote") == YES_VOTE)
    no_count = sum(1 for entry in named if entry.get("vote") == NO_VOTE)
...
class Application:
    def __init__(
        ...
        locale_provider=lambda: SOURCE_LOCALE,
    ...
    def _setup_group(self, parts: list[str]) -> None:
        ...
        if command in {"page-previous", "page-next"}:
            ...
            persistence = (
                DATASPACE_SESSION_PERSISTENCE
                if self.state.game_settings is not None
    ...
    def _open_exact_card(self, card_id: str) -> None:
        ...
        self._navigate(Route.SETUP_EXACT_CARD_PREVIEW)
    ...
    def _activate_prefs(self, key: str) -> None:
        ...
        if command in {"page-previous", "page-next"}:
            ...
            current = (
                preferences.locale
                if preferences.locale in locales
                else AUTOMATIC_LOCALE
            )
            selected = self._choice(
                strings.UI_LANGUAGE,
                tuple(
                    strings.Text.message(LOCALE_LABEL_IDS[locale])
    ...
    def private_vote_shortcut(self, vote: str) -> bool:
        ...
        if (
            ...
            or not player_id
            or vote not in NEVER_HAVE_I_EVER_VOTE_VALUES
        ):
            return False
