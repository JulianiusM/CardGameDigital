...
SERVER_VALIDATE_OPERATION = 'server.validate'
METADATA_LOAD_OPERATION = 'metadata.load'
...
def reduce(state: AppState, incoming: Action) -> Transition:
    handler = _ACTION_HANDLERS.get(incoming.kind)
    if handler is None:
        return _transition(state)
    return handler(state, incoming)


def _reduce_start(state: AppState, _incoming: Action) -> Transition:
...
def _reduce_storage_loaded(state: AppState, incoming: Action) -> Transition:
    ...
    if configured_server_origin:
        next_state = bump(
            ...
            servers=servers,
            recovery=recovery,
            busy_operation=SERVER_VALIDATE_OPERATION,
...
def _reduce_storage_failed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(bump(state, route=Route.SERVER_LIST, notification=_notice(payload, strings.STORAGE_ERROR)))


def _reduce_discovery_started(state: AppState, _incoming: Action) -> Transition:
    return _transition(bump(state, transport_state="DISCOVERING"))


def _reduce_discovery_requested(state: AppState, _incoming: Action) -> Transition:
    return _transition(
...
def _reduce_server_discovered(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    candidate = payload["server"]
    return _transition(bump(state, servers=_merge_server(state.servers, candidate)))


def _reduce_server_unavailable(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_discovery_finished(state: AppState, _incoming: Action) -> Transition:
    transport = "CONNECTED" if state.server_info else "DISCONNECTED"
    return _transition(bump(state, transport_state=transport))


def _reduce_select_server(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    next_state = bump(
        state,
...
def _reduce_server_details_opened(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    server_id = str(payload["server_id"])
...
def _reduce_server_validated(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    candidate = payload["server"]
    ...
    next_state = bump(
        ...
        busy_operation=METADATA_LOAD_OPERATION,
        notification=None,
        profiles=(),
        groups=(),
        locales=(),
...
def _reduce_server_validation_failed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_metadata_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_metadata_partial(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_servers_replaced(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_navigate(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    route = payload["route"]
...
def _reduce_help_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_help_topic_selected(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_help_document_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    document = payload["document"]
...
def _reduce_help_page_changed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(bump(state, help_page=max(0, int(payload["page"]))))


def _reduce_collection_page_changed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(bump(state, collection_page=max(0, int(payload["page"]))))


def _reduce_back(state: AppState, _incoming: Action) -> Transition:
...
def _reduce_begin_setup(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_restart_setup(state: AppState, _incoming: Action) -> Transition:
    if not state.setup:
        return _transition(state)
    setup = replace(
        state.setup,
...
def _reduce_setup_replaced(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    setup = payload["setup"]
...
def _reduce_policy_editor_target(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    setup = state.setup
    if setup:
        setup = replace(
            setup,
            selected_rule_id=payload.get("rule_id", setup.selected_rule_id),
            selected_card_id=payload.get("card_id", setup.selected_card_id),
            policy_facet=payload.get("facet", setup.policy_facet),
        )
    return _transition(bump(state, setup=setup))


def _reduce_taxonomy_requested(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_taxonomy_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_eligibility_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_card_searched(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    if not state.setup:
        return _transition(state)
    return _transition(
...
def _reduce_search_cards_requested(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    if not state.setup:
        return _transition(state)
    setup = replace(
...
def _reduce_create_couch_requested(state: AppState, _incoming: Action) -> Transition:
    if not state.setup:
        return _transition(state)
    return _transition(
...
def _reduce_couch_command_requested(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_couch_resync_requested(state: AppState, _incoming: Action) -> Transition:
    if not state.couch_snapshot:
        return _transition(state)
    return _transition(
...
def _reduce_create_room_requested(state: AppState, _incoming: Action) -> Transition:
    if not state.setup:
        return _transition(state)
    return _transition(
...
def _reduce_join_room_requested(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
...
def _reduce_create_group_requested(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_update_group_requested(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_recover_requested(state: AppState, _incoming: Action) -> Transition:
...
def _reduce_discard_recovery(state: AppState, _incoming: Action) -> Transition:
...
def _reduce_clear_data_requested(state: AppState, _incoming: Action) -> Transition:
    return _transition(
        bump(
            state,
            preferences=Preferences(),
...
def _reduce_clear_recent_servers(state: AppState, _incoming: Action) -> Transition:
    next_state = bump(
        state,
        route=Route.SERVER_LIST,
        route_stack=(),
...
def _reduce_operation_started(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_couch_created(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _page_resets(state: AppState, automatic: bool, collection: bool, card: bool) -> dict[str, int]:
    return {
        "auto_page": 0 if automatic else state.auto_page,
        "auto_page_epoch": state.auto_page_epoch + 1 if automatic else state.auto_page_epoch,
        "collection_page": 0 if collection else state.collection_page,
        "card_page": 0 if card else state.card_page,
    }


def _reduce_couch_updated(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    snapshot = payload["snapshot"]
    ...
    selected_vote, vote_phase = _updated_couch_vote(state, snapshot)
    ...
    next_state = bump(
        ...
        couch_pool_exhausted=False,
        couch_end_destination=None,
        **_page_resets(state, reset_result_page, reset_voter_page, reset_card_page),
    )
    if ended:
        return _transition(next_state, effect("CLEAR_RECOVERY", owner="storage-recovery"))
    return _transition(next_state)

def _updated_couch_vote(state, snapshot):
    ...
    if selected_vote is None:
        ...
        vote_phase = "CHOICE"
    return selected_vote, vote_phase


def _reduce_couch_vote_player_selected(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_couch_vote_cancelled(state: AppState, _incoming: Action) -> Transition:
...
def _reduce_room_joined(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_ws_state(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_ws_envelope(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _websocket_event(state, payload["envelope"])


def _reduce_qr_ready(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_room_closed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_leave_active(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    if state.active_room:
        return _transition(
            bump(
...
def _reduce_end_couch_for_setup(state: AppState, _incoming: Action) -> Transition:
...
def _reduce_leave_completed(state: AppState, _incoming: Action) -> Transition:
    reference = state.active_room.credential_reference if state.active_room else None
...
def _reduce_preferences_changed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_diagnostics_requested(state: AppState, _incoming: Action) -> Transition:
    return _transition(state, effect("LOAD_DIAGNOSTICS", owner="diagnostics"))


def _reduce_diagnostics_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(bump(state, diagnostics=tuple(payload.get("entries", ()))))


def _reduce_groups_loaded(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    route = payload.get("route", state.route)
    destination = route
...
def _reduce_group_draft_started(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_group_draft_updated(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_authorization_updated(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(bump(state, authorization=payload["authorization"], busy_operation=None))


def _reduce_auth_link_requested(state: AppState, _incoming: Action) -> Transition:
    return _transition(
...
def _reduce_authorization_pending(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(
        bump(
            state,
...
def _reduce_authorization_complete(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    next_state = bump(
        state,
        authorization=payload["authorization"],
...
def _reduce_auth_cancel_requested(state: AppState, _incoming: Action) -> Transition:
    return _transition(
        bump(
            state,
            authorization=None,
...
def _reduce_authorization_cancelled(state: AppState, _incoming: Action) -> Transition:
    return _transition(bump(state, authorization=None, qr_path=None, busy_operation=None))


def _reduce_auth_unlink_requested(state: AppState, _incoming: Action) -> Transition:
    return _transition(
...
def _reduce_authorization_unlinked(state: AppState, _incoming: Action) -> Transition:
    setup = state.setup
...
def _reduce_confirm(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    confirmation = Confirmation(
...
def _reduce_cancel_confirmation(state: AppState, _incoming: Action) -> Transition:
    return _transition(bump(state, confirmation=None))


def _reduce_notify(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
    return _transition(bump(state, notification=_notice(payload)))


def _reduce_clear_notification(state: AppState, _incoming: Action) -> Transition:
    return _transition(bump(state, notification=None))


def _reduce_operation_failed(state: AppState, incoming: Action) -> Transition:
    payload = incoming.payload
...
def _reduce_stop(state: AppState, _incoming: Action) -> Transition:
...
_ACTION_HANDLERS = {
    "START": _reduce_start,
    "STORAGE_LOADED": _reduce_storage_loaded,
    "STORAGE_FAILED": _reduce_storage_failed,
    "DISCOVERY_STARTED": _reduce_discovery_started,
    "DISCOVERY_REQUESTED": _reduce_discovery_requested,
    "SERVER_DISCOVERED": _reduce_server_discovered,
    "SERVER_UNAVAILABLE": _reduce_server_unavailable,
    "DISCOVERY_FINISHED": _reduce_discovery_finished,
    "SELECT_SERVER": _reduce_select_server,
    "SERVER_DETAILS_OPENED": _reduce_server_details_opened,
    "SERVER_VALIDATED": _reduce_server_validated,
    "SERVER_VALIDATION_FAILED": _reduce_server_validation_failed,
    "METADATA_LOADED": _reduce_metadata_loaded,
    "METADATA_PARTIAL": _reduce_metadata_partial,
    "SERVERS_REPLACED": _reduce_servers_replaced,
    "NAVIGATE": _reduce_navigate,
    "HELP_LOADED": _reduce_help_loaded,
    "HELP_TOPIC_SELECTED": _reduce_help_topic_selected,
    "HELP_DOCUMENT_LOADED": _reduce_help_document_loaded,
    "HELP_PAGE_CHANGED": _reduce_help_page_changed,
    "COLLECTION_PAGE_CHANGED": _reduce_collection_page_changed,
    "BACK": _reduce_back,
    "BEGIN_SETUP": _reduce_begin_setup,
    "RESTART_SETUP": _reduce_restart_setup,
    "SETUP_REPLACED": _reduce_setup_replaced,
    "POLICY_EDITOR_TARGET": _reduce_policy_editor_target,
    "TAXONOMY_REQUESTED": _reduce_taxonomy_requested,
    "TAXONOMY_LOADED": _reduce_taxonomy_loaded,
    "ELIGIBILITY_LOADED": _reduce_eligibility_loaded,
    "CARD_SEARCHED": _reduce_card_searched,
    "SEARCH_CARDS_REQUESTED": _reduce_search_cards_requested,
    "CREATE_COUCH_REQUESTED": _reduce_create_couch_requested,
    "COUCH_COMMAND_REQUESTED": _reduce_couch_command_requested,
    "COUCH_RESYNC_REQUESTED": _reduce_couch_resync_requested,
    "CREATE_ROOM_REQUESTED": _reduce_create_room_requested,
    "JOIN_ROOM_REQUESTED": _reduce_join_room_requested,
    "CREATE_GROUP_REQUESTED": _reduce_create_group_requested,
    "UPDATE_GROUP_REQUESTED": _reduce_update_group_requested,
    "RECOVER_REQUESTED": _reduce_recover_requested,
    "DISCARD_RECOVERY": _reduce_discard_recovery,
    "CLEAR_DATA_REQUESTED": _reduce_clear_data_requested,
    "CLEAR_RECENT_SERVERS": _reduce_clear_recent_servers,
    "OPERATION_STARTED": _reduce_operation_started,
    "COUCH_CREATED": _reduce_couch_created,
    "COUCH_UPDATED": _reduce_couch_updated,
    "COUCH_VOTE_PLAYER_SELECTED": _reduce_couch_vote_player_selected,
    "COUCH_VOTE_CANCELLED": _reduce_couch_vote_cancelled,
    "ROOM_JOINED": _reduce_room_joined,
    "WS_STATE": _reduce_ws_state,
    "WS_ENVELOPE": _reduce_ws_envelope,
    "QR_READY": _reduce_qr_ready,
    "ROOM_CLOSED": _reduce_room_closed,
    "LEAVE_ACTIVE": _reduce_leave_active,
    "END_COUCH_FOR_SETUP": _reduce_end_couch_for_setup,
    "LEAVE_COMPLETED": _reduce_leave_completed,
    "PREFERENCES_CHANGED": _reduce_preferences_changed,
    "DIAGNOSTICS_REQUESTED": _reduce_diagnostics_requested,
    "DIAGNOSTICS_LOADED": _reduce_diagnostics_loaded,
    "GROUPS_LOADED": _reduce_groups_loaded,
    "GROUP_DRAFT_STARTED": _reduce_group_draft_started,
    "GROUP_DRAFT_UPDATED": _reduce_group_draft_updated,
    "AUTHORIZATION_UPDATED": _reduce_authorization_updated,
    "AUTH_LINK_REQUESTED": _reduce_auth_link_requested,
    "AUTHORIZATION_PENDING": _reduce_authorization_pending,
    "AUTHORIZATION_COMPLETE": _reduce_authorization_complete,
    "AUTH_CANCEL_REQUESTED": _reduce_auth_cancel_requested,
    "AUTHORIZATION_CANCELLED": _reduce_authorization_cancelled,
    "AUTH_UNLINK_REQUESTED": _reduce_auth_unlink_requested,
    "AUTHORIZATION_UNLINKED": _reduce_authorization_unlinked,
    "CONFIRM": _reduce_confirm,
    "CANCEL_CONFIRMATION": _reduce_cancel_confirmation,
    "NOTIFY": _reduce_notify,
    "CLEAR_NOTIFICATION": _reduce_clear_notification,
    "OPERATION_FAILED": _reduce_operation_failed,
    "STOP": _reduce_stop,
}
...
def _websocket_event(state: AppState, envelope: dict[str, Any]) -> Transition:
    ...
    if event_type == "server.hello":
        return _handle_server_hello(state, payload)
    if event_type == "room.snapshot":
        return _websocket_event_room_snapshot(payload, state)
...
def _handle_server_hello(state, payload):
    if payload.get("role") != "DISPLAY":
        return _transition(
            bump(
                state,
...
def _websocket_event_room_snapshot(payload, state):
    snapshot = payload
    ...
    notification = _room_snapshot_notification(state, snapshot, settings_changed_by_other)
    settings = snapshot.get("settings")
    player_count = _room_player_count(snapshot)
    room_eligibility, signature, preview_effects = _room_eligibility_effects(state, session, settings, player_count)
    ...
    next_state = bump(
        ...
        **_page_resets(state, reset_auto_page, False, reset_card_page),
    )
    return _transition(next_state, *preview_effects, *taxonomy_effects)

def _room_snapshot_notification(state, snapshot, settings_changed_by_other):
    previous_host_state = (
...
def _room_eligibility_effects(state, session, settings, player_count):
    signature = None
    ...
    return room_eligibility, signature, preview_effects
