...
class FocusAndPresentationTests(unittest.TestCase):
    ...
    def test_exact_card_preview_pages_complete_copy_before_directive_editing(self) -> None:
        card_id = "00000000-0000-4000-8000-000000000001"
        prefix = "START OF EXACT CARD. "
        suffix = " END OF EXACT CARD."
        ...
        self.assertEqual(first.view_mode, "card")
        self.assertGreater(first.card_page_count, 1)
        self.assertTrue((first.card_text.literal or "").startswith("START OF EXACT CARD."))
        self.assertTrue((last.card_text.literal or "").endswith("END OF EXACT CARD."))
        self.assertEqual(last.card_page_count, first.card_page_count)
        self.assertEqual(
            [entry.key for entry in first.actions],
    ...
    def test_group_preview_and_server_origin_insert_wraps_without_losing_text(self) -> None:
        group_name = "G" * 80
        members = tuple(f"M{index}" + ("W" * 37) for index in range(4))
        group_view = present(
            AppState(
                route=Route.GROUP_LIST,
                groups=({"id": "group", "name": group_name, "members": members},),
            )
        )
        group = group_view.items[0]
        self.assertEqual(group.label.literal, group_name)
        compact_preview = (group.secondary.literal or "").replace("\n", "")
        for member in members[:3]:
            self.assertIn(member, compact_preview)
        self.assertTrue((group.secondary.literal or "").endswith("· +1"))

        origin = f"https://{'h' * 63}.example.test:30443"
        server = ServerRecord("server", origin, "S" * 80, "local", "manual", 1)
        server_view = present(AppState(route=Route.SERVER_LIST, servers=(server,)))
        projected_origin = server_view.items[0].secondary.literal or ""
        self.assertEqual(projected_origin.replace("\n", ""), origin)
        self.assertTrue(all(len(line) <= 38 for line in projected_origin.splitlines()))
    ...
    def test_couch_sync_recovery_prioritizes_complete_instruction_below_the_card(self) -> None:
        player = {"id": "player-1", "name": "Current player"}
        view = present(
            AppState(
                route=Route.COUCH_GAME,
                ...
                couch_snapshot={
                    ...
                    "currentCard": {
                        "id": "card-1",
                        "cardText": "The last confirmed Card remains visible.",
                        "cardType": "QUESTION",
        ...
        self.assertEqual(view.body.message_id, strings.COUCH_SYNC_BODY)
        self.assertEqual(
            [entry.key for entry in view.actions], ["couch:resync", "couch:end"]
        )
        self.assertEqual(
            (view.card_text.literal or "").replace("\n", ""),
            "The last confirmed Card remains visible.",
        )
        self.assertEqual(view.current_player.literal, "")
