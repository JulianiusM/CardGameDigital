...
from lib import app as app_module
from lib import effects as effects_module
from lib.protocol import validation as protocol_validation
...
from lib.storage import profile_store
...
class ProtocolValidationTests(unittest.TestCase):
    def test_native_consumers_reuse_generated_protocol_constants(self) -> None:
        ...
        self.assertEqual(effects_module.PROTOCOL_VERSION, PROTOCOL_VERSION)
        self.assertEqual(profile_store.PROTOCOL_VERSION, PROTOCOL_VERSION)

    def test_native_protocol_enum_sets_and_named_values_match_generated_order(self) -> None:
        generated = load_generated_json("enums.json")
        projections = (
            ("cardLifecycles", "CARD_LIFECYCLE_ORDER", "CARD_LIFECYCLES"),
            (
                "sessionPersistenceModes",
                "SESSION_PERSISTENCE_ORDER",
                "SESSION_PERSISTENCE_MODES",
            ),
            (
                "neverHaveIEverVoteStatuses",
                "NEVER_HAVE_I_EVER_VOTE_STATUS_ORDER",
                "NEVER_HAVE_I_EVER_VOTE_STATUSES",
            ),
            (
                "neverHaveIEverVoteValues",
                "NEVER_HAVE_I_EVER_VOTE_VALUE_ORDER",
                "NEVER_HAVE_I_EVER_VOTE_VALUES",
            ),
            (
                "participantLeftReasons",
            ...
            (
                "participantConnectionStatuses",
                "PARTICIPANT_CONNECTION_STATUS_ORDER",
                "PARTICIPANT_CONNECTION_STATUSES",
            ),
            (
                "neverHaveIEverRevealModes",
                "NEVER_HAVE_I_EVER_REVEAL_MODE_ORDER",
                "NEVER_HAVE_I_EVER_REVEAL_MODES",
            ),
        )
        for generated_key, order_name, set_name in projections:
            with self.subTest(enum=generated_key):
                expected_order = tuple(generated[generated_key])
                self.assertEqual(getattr(protocol_validation, order_name), expected_order)
                self.assertEqual(getattr(protocol_validation, set_name), frozenset(expected_order))

        self.assertEqual(
            (
                protocol_validation.ANONYMOUS_REVEAL_MODE,
                protocol_validation.NAMED_REVEAL_MODE,
            ),
            protocol_validation.NEVER_HAVE_I_EVER_REVEAL_MODE_ORDER,
        )
        self.assertIs(
            app_module.NEVER_HAVE_I_EVER_REVEAL_MODE_ORDER,
            protocol_validation.NEVER_HAVE_I_EVER_REVEAL_MODE_ORDER,
        )
