...
class ReleaseVerifierTests(unittest.TestCase):
    def _verify(
        ...
        extra_entries: Optional[Mapping[str, str]] = None,
...
"""
    ...
    def test_rejects_ignored_local_test_profile_data(self) -> None:
        manifest = (CLIENT_ROOT / "addon.xml").read_text(encoding="utf-8")

        result = self._verify(
            manifest,
            extra_entries={
                f"{ADDON_ID}/.test-profile/client.json": '{"servers": []}\n'
            },
        )

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Forbidden release entry", result.stdout + result.stderr)
