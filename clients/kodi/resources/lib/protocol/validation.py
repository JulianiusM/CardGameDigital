...
CARD_TYPES = {"QUESTION", "DARE", "CONVERSATION_META"}
ROOM_ROLES = {"HOST", "PLAYER", "DISPLAY"}
ROLE_CHANGE_REASONS = {
...
def _validate_configuration(value: Any, name: str) -> Mapping[str, Any]:
    ...
    starting = _integer(configuration.get("startingIntensity"), f"{name}.startingIntensity", 1, 5)
    maximum = _integer(configuration.get("maximumIntensity"), f"{name}.maximumIntensity", 1, 5)
    if starting > maximum:
        raise ProtocolViolation(f"{name}.startingIntensity exceeds maximumIntensity")
    if configuration.get("intensityProgressionUnit") not in {"ROUNDS", "CARDS"}:
    ...
    if not (increment * 2).is_integer():
        raise ProtocolViolation(f"{name}.intensityProgressionIncrement must use half steps")
    _number(configuration.get("randomQuestionRatio"), f"{name}.randomQuestionRatio", 0, 1)
    _integer(configuration.get("maximumTypeStreak"), f"{name}.maximumTypeStreak", 1, 10)
    _integer(
...
def validate_game_settings(value: Any) -> dict[str, Any]:
    ...
    starting = _integer(settings.get("startingIntensity"), "game settings.startingIntensity", 1, 5)
    maximum = _integer(settings.get("maximumIntensity"), "game settings.maximumIntensity", 1, 5)
    if starting > maximum:
    ...
    if settings.get("intensityProgressionUnit") not in {"ROUNDS", "CARDS"}:
        raise ProtocolViolation("game settings intensityProgressionUnit is invalid")
    _integer(settings.get("intensityProgressionInterval"), "game settings progression interval", 1, 100)
    increment = _number(
        settings.get("intensityProgressionIncrement"), "game settings progression increment", 0.5, 4
    )
    if not (increment * 2).is_integer():
        raise ProtocolViolation("game settings progression increment must use half steps")
    _number(settings.get("randomQuestionRatio"), "game settings question ratio", 0, 1)
    _integer(settings.get("letsTalkMetaInterval"), "game settings conversation interval", 1, 100)
...
def validate_card_search(value: Any, maximum_cards: int = 50) -> dict[str, Any]:
    ...
    for index, value_card in enumerate(cards):
        ...
        if card.get("lifecycle") not in {"ACTIVE", "RETIRED"}:
...
def validate_couch_snapshot(value: Any) -> dict[str, Any]:
    ...
    if snapshot.get("persistence") not in {"EPHEMERAL", "DATASPACE"}:
    ...
    if settings.get("neverHaveIEverRevealMode") not in {
        "ANONYMOUS_AGGREGATE",
        "NAMED_ANSWERS",
    }:
...
def _validate_couch_voting(value: Any, player_ids: set[str]) -> None:
    ...
    if voting.get("revealMode") not in {"ANONYMOUS_AGGREGATE", "NAMED_ANSWERS"}:
    ...
    for entry in progress:
        ...
        if current.get("status") not in {"PENDING", "VOTED"}:
    ...
    for entry in named:
        ...
        if current.get("vote") not in {"YES", "NO"}:
...
def _validate_snapshot(value: Mapping[str, Any]) -> None:
    ...
    if value.get("bootstrapMode") not in {"CREATOR_HOST", "DISPLAY_WAITING_FOR_HOST"}:
    ...
    for participant in participants:
        ...
        if current.get("role") not in {"HOST", "PLAYER", "DISPLAY"}:
            raise ProtocolViolation("participant.role is invalid")
        _string(current.get("displayName"), "participant.displayName", 1, 40)
        if current.get("connectionStatus") not in {"CONNECTED", "TEMPORARILY_DISCONNECTED"}:
