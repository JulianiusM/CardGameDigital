...
def reduce(state: AppState, incoming: Action) -> Transition:
    ...
    if kind == "AUTHORIZATION_UNLINKED":
        ...
        if setup and setup.group_id:
            setup = replace(setup, group_id=None, persistence="EPHEMERAL")
        next_state = bump(
            state,
            authorization=None,
            groups=(),
...
def _websocket_event(state: AppState, envelope: dict[str, Any]) -> Transition:
    ...
    if event_type == "room.snapshot":
        ...
        if (
            host_status.get("state") == "CONNECTED"
            and previous_host_state
            in {"AWAITING_FIRST_HOST", "AWAITING_REPLACEMENT_HOST", "RECONNECTING"}
        ):
    ...
    if event_type == "room.participantLeft":
        ...
        message_id = (
            ...
            if payload.get("reason") == "DISCONNECT_EXPIRED"
...
def _named_result_signature(snapshot: Any) -> Any:
    ...
    if not isinstance(voting, dict) or voting.get("revealMode") != "NAMED_ANSWERS":
        return None
