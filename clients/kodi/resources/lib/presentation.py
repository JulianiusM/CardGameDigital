...
def _server_item(server: ServerRecord, state: AppState) -> ViewItem:
    ...
    return ViewItem(
        ...
        secondary=raw(_wrapped_identifier(server.origin)),
        detail=msg(status),
    ...
    if setup.mode == "NEVER_HAVE_I_EVER":
        entries.append(
            item(
                ...
                secondary=msg(
                    strings.NAMED
                    if setup.never_reveal_mode == "NAMED_ANSWERS"
...
def _option_label(context: str, value: object) -> Text:
    ...
    labels = {
        ...
        "ANONYMOUS_AGGREGATE": strings.ANONYMOUS,
        "NAMED_ANSWERS": strings.NAMED,
        "ACTIVE": strings.ACTIVE,
        "RETIRED": strings.RETIRED,
...
def _named_result_columns(
    ...
    yes = tuple(
        ...
        for entry in named
        if entry.get("vote") == "YES"
    )
    no = tuple(
        str(entry.get("displayName", ""))
        for entry in named
        if entry.get("vote") == "NO"
    )
    total_pages = max(
        1,
...
def _host_status(snapshot: Mapping[str, Any]) -> Text:
    ...
    if host_state == "CONNECTED":
        return msg(strings.HOST_CONNECTED, name)
    if host_state in ("RECONNECTING", "CONNECTING"):
        return msg(strings.HOST_RECONNECTING, name)
    if host_state == "AWAITING_REPLACEMENT_HOST":
    ...
    if settings.get("mode") == "NEVER_HAVE_I_EVER":
        lobby_facts.append(
            ViewFact(
                ...
                msg(
                    strings.NAMED
                    if settings.get("neverHaveIEverRevealMode") == "NAMED_ANSWERS"
                    else strings.ANONYMOUS
                ),
            )
        )
        ...
        result.append(
            {
                ...
                "connected": participant.get("connectionStatus") == "CONNECTED",
            }
        )
        result.extend(
            {
...
def _room_stage_body(
    ...
    if isinstance(voting, Mapping):
        ...
        voted = sum(1 for entry in progress if entry.get("status") == "VOTED")
...
def _preferences(state: AppState) -> ViewModel:
    preferences = state.preferences
    locale_label = {
        "auto": strings.LANGUAGE_AUTO,
        "en-GB": strings.LANGUAGE_ENGLISH,
        "de-DE": strings.LANGUAGE_GERMAN,
    }.get(preferences.locale, strings.LANGUAGE_AUTO)
    entries: list[ViewItem] = [
