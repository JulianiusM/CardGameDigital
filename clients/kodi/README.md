...
For an unpacked development install, link or copy `clients/kodi` to Kodi's add-ons
...
unpacked copy of `clients/kodi` at Kodi's exact add-on path
...
```powershell
$sourceClient = (Resolve-Path "clients/kodi").Path
...
```powershell
powershell -File clients/kodi/tools/kodi_visual_driver.ps1 LaunchAddon `
    -OutputDirectory artifacts/kodi-dpad-verification
powershell -File clients/kodi/tools/kodi_visual_driver.ps1 LaunchFixture `
...
```powershell
python -B clients/kodi/tools/visual_fixture.py --check
python -B -m unittest discover -s clients/kodi/tests `
