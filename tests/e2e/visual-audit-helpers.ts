...
export async function captureVisualAudit(
    ...
    const findings = await page.evaluate((auditOptions) => {
        ...
        const descendantTextRects = (element: HTMLElement): DOMRect[] => {
            ...
            while (node) {
                ...
                if (node.textContent?.trim() && parent && isVisible(parent)) {
                    const range = document.createRange();
                    range.selectNodeContents(node);
                    collectClippedTextRects(range, parent);
                }
                node = walker.nextNode();
            }
            return rects;

            function collectClippedTextRects(range: Range, parent: HTMLElement) {
        ...
        function auditPairOverlaps() {
            for (let index = 0; index < overlapCandidates.length; index += 1) {
                ...
                for (
                    ...
                    peerIndex < overlapCandidates.length;
                    peerIndex += 1
                ) {
                    auditElementOverlap(peerIndex, first, firstIsInteractive, firstRects);
                }
            }
            ...
            function auditElementOverlap(
                peerIndex: number,
                first: HTMLElement,
                firstIsInteractive: boolean,
                firstRects: DOMRect[],
            ) {
                const second = overlapCandidates[peerIndex];
                if (first.contains(second) || second.contains(first)) return;
                if (insideHorizontalScroller(first) || insideHorizontalScroller(second)) return;
                const firstScroller = scrollingAncestor(first);
                const secondScroller = scrollingAncestor(second);
                if (firstScroller !== secondScroller && (firstScroller || secondScroller)) return;
                ...
                let overlapWidth = 0;
                let overlapHeight = 0;
                ({ overlapWidth, overlapHeight } = measureOverlap(
                    firstRects,
                    secondRects,
                    overlapWidth,
                    overlapHeight,
                ...
                if (overlapWidth <= minimumOverlap || overlapHeight <= minimumOverlap) return;
                overlaps.push({
        ...
        function isEmptyVisiblePlaceholder(
            control: HTMLInputElement | HTMLTextAreaElement,
        ): boolean {
            return isVisible(control) && !control.value && Boolean(control.placeholder.trim());
        }
        ...
        function auditPlaceholders() {
            for (const control of document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>(
                ...
                auditPlaceholderControl(control);
            }

            function auditPlaceholderControl(control: HTMLInputElement | HTMLTextAreaElement) {
                if (!isEmptyVisiblePlaceholder(control)) return;
        ...
        function auditSelectedTabVisibility() {
            for (const selectedTab of document.querySelectorAll<HTMLElement>(
                ...
                const buttonEscapes =
                    selectedBounds.left < visibleLeft - 1 ||
                    selectedBounds.right > visibleRight + 1;
                ...
                if (
                    ...
                    if (escapedTextRects.length > 0) {
                        ...
                        const paintedRight = Math.max(
                            ...escapedTextRects.map((rect) => rect.right),
                        );
                        const paintedBottom = Math.max(
                            ...escapedTextRects.map((rect) => rect.bottom),
                        );
                        truncations.push({
                            selector: selectorFor(element),
        ...
        function auditPaintedText() {
            for (const element of allVisible) {
                auditElementBounds(element);
            }

            function auditElementBounds(element: HTMLElement) {
                const bounds = element.getBoundingClientRect();
                if (
