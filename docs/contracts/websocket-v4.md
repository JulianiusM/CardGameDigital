# WebSocket protocol v4 contract

## Connection and identity

Connection/work counts, timeouts, rate windows and queue/scheduling budgets below are
the defaults from [operational settings](operational-settings.md). They may be tuned
without changing v4. The per-message byte limit remains mandatory.

Connect to `/ws`. Every strict envelope contains `protocol: 4`, `type`, `requestId`,
`revision`, and `payload`. The first message is `client.hello` with
`supportedProtocolVersions: [4]`, Room code, and the participant reconnect credential.
...
The process supports at most 2,000 simultaneous connections. Clients have five seconds
to submit hello. At most 32 authentications may be in
progress; admitted authentication has a separate 30-second deadline. Busy authentication
closes with retryable `1013`. Shared reconnect/presence refreshes may coalesce over
50 ms, while each successful handshake still receives its own authoritative snapshot.
Public fields are encoded once per broadcast; viewer controls remain individually
projected. A shared UTF-8 prefix and a private tail may use two text fragments; together
they remain one complete JSON envelope under the existing aggregate message limit.
Presence and snapshot fan-out yield after 16 recipients or 4 MiB of queued bytes;
unchanged presence is omitted and newer gameplay can interrupt obsolete ambient
presence/snapshot delivery. Explicit request-correlated snapshots still complete. These scheduling
changes preserve complete v4 envelopes, revision ordering and private-answer masking.
...
Game commands are processed sequentially per socket. Messages are JSON text limited to **4,194,304
UTF-8 bytes in either direction**, inclusive of the complete JSON envelope. Fragmented
messages use the same aggregate limit. All v4 clients must receive this allowance;
it is not negotiated down. `requestId` is null or 1–128 characters. Configuration and
boundary taxonomy arrays contain unique members of their declared enums.
...
`server.pong` with `{serverTime}`. A validated authenticated ping of at most 1 KiB
bypasses queued game work, while still consuming the public socket rate budget.
Missing application heartbeat responses cause the
...
retained from protocol v3; snapshots always include them.
...
`cardFallbackEnabled` and `cardFallbackLocales` were added in protocol v3 and
...
correction retained from protocol v3: field names/types and the documented privacy semantics
...
Protocol v4 replaces v3 to guarantee the shared receive capacity. Early v3 native builds
accepted only 64 KiB and at most 500 participants, while later v3 builds accepted larger
messages. That ambiguity cannot be resolved by keeping the same version. Server, browser
and generated Kodi definitions now select v4; discovery and `/server-info` advertise it.
Clients supporting only v1/v2/v3 receive `PROTOCOL_VERSION_UNSUPPORTED` and must upgrade.
This change retains the phase 3 live-catalog design, sparse policy, bounded queues and
runtime v7. It adds no catalog snapshots, delivery pages, truncation or legacy fallback.

The enrollment requirement introduced in v3 remains: first-time late joiners must send
`command.setBoundaries` before becoming Session players. Other JSON field meanings are
unchanged. The stricter request-ID and enum-list validation rejects previously accepted
unbounded or duplicate-only inputs that could undermine the receive guarantee.
The `currentCard.cardIntensity` response field retains its relative Card meaning;
`currentCard.intensity` retains its global meaning.
The defaulted `settings.cardPolicy` contains only pending-Session policy. Catalogs and captured
...
`session.remainingCardCount` remains a display-only field.
`bootstrapMode`, `hostStatus`, and the expanded `room.roleChanged` payload retain their
existing meanings. Role authority remains server-owned; clients must use `server.hello` and
the latest snapshot instead of assuming that every Room creator is Host.

### Capacity correction (2026-09-06)

Server, web and generated Kodi definitions use the shared 4 MiB message allowance,
including snapshots at the configured ceilings of 1,000 participants and 1,000 represented
players. Default Room capacity remains 100 for each. A displayed Card is bounded by the
catalog contract (4,000 Unicode characters and 8,192 UTF-8 bytes). Settings carry at most
250 rules and 1,000 exact overrides, including maximum predicates/directives and names.
The capacity calculation includes named voting results, device-player rows, escaping and
the envelope; catalogs and captured persistent policy remain off the wire.
Input queues and send buffers are bounded as specified in the
