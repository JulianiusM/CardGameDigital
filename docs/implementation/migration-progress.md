...
| 9 — Presentation/release           | **IMPLEMENTED** | Local audio, animation/reduced-motion/accessibility controls, German message catalog, shared Golden Mischief tokens, portable/public server packaging, and the independent native Kodi client with generated contracts/assets, Couch/Room flows, discovery, recovery, localization, tests, reproducible ZIP, checksum, SBOM, provenance, and release workflow; ADR-010. | Portable server archives must still be produced on every supported OS/architecture. Kodi clean-install and platform/skin/remote/network QA remain external release gates; native account linking is capability-gated and disabled by the current server. Android-family clients remain deferred.                                                                                 |
...
| 11 — Scoped Card management        | **VERIFIED**    | Catalog v2 and policy migrations `1787351000000`–`1787352000000`; core resolver/compiler and explicit sensitivity gate; authoritative eligibility preview; application/repository/HTTP adapters; Session runtime v4; Golden Mischief management workspace; ADR-011 and localized Help.                                                                                  | Unit, simulation, SQLite/HTTP, public MariaDB, and Playwright coverage pass for precedence, ownership, revisions, import, preview, sensitivity, cursor/bulk behavior, quick-game compatibility, 1,000-entry UI paging, narrow-phone overflow, and 10,000/50,000-Card compilation.                                                                                                |
| 12 — Source-layout consolidation   | **IMPLEMENTED** | `apps/{server,web,kodi}`; top-level `packages`; concern-specific `tooling`; persistence-owned entities; canonical protocol DTOs; generated web/Kodi presentation data and Kodi localization; ADR-013.                                                                                                                                                                   | Repository paths changed without changing runtime contracts or release-unit ownership. Architecture, generation, server/web/native, packaging, and browser workflow checks guard the new boundaries.                                                                                                                                                                             |
...
- Early package seams originally lived under `src/packages` to avoid a premature
  workspace move. ADR-013 completed their relocation to top-level `packages`;
  `game-core` still contains no Express, TypeORM, WebSocket, or UI imports.
...
- Maximum social sensitivity is an independent ordered profile/game-setting gate.
  Child-friendly behavior is a normal editable built-in profile combining conservative
  taxonomy, operational, intensity, and sensitivity defaults; there is no parallel
  preset-activation model.
...
- `game-card-catalog/v2` adds producer-owned social sensitivity and atomic player-count
  defaults/overrides without deriving either from localized text. Runtime reconciliation
  remains a non-destructive FULL apply.
- DataSpace, Group, and pending-Session policy use the same sparse directives and
  deterministic resolver. A version-4 Session snapshot freezes effective values and
  provenance at start while roster-dependent player-count checks remain authoritative at
  draw time.
- Local none-auth and authenticated public deployments use the same application and
  repository contracts. Their only difference is ownership resolution.
- The browser presents inherited defaults first, creates disabled rule drafts, requires
  a current bounded preview before save, and uses inline confirmations for replacement
  or destructive actions.
- Card transport is cursor-paged; DataSpace, Group, and rule ownership lists are
  search-filtered into bounded rendered pages. Playwright verifies 1,000-entry lists and
  narrow-phone overflow behavior.
- Pending setup previews eligible Cards on the server at both start and maximum
  intensity, including policy, localization, Group history, and player count while
  excluding private boundaries.
- The adaptive background uses derived global Card intensity inside the selected taxonomy
  family and preserves it between Cards. Card surfaces show relative Card and derived
  global intensity as distinct symbols with accessible labels.
