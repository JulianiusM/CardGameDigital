...
- The server projects an authoritative snapshot separately for each participant. Only the active participant receives the Classic choice control, displays receive no command capabilities, and individual Never-Have-I-Ever vote values are never broadcast.
...
- The shared display stays visually focused on lobby, active player, card, and aggregate results while phones expose only relevant controls.
