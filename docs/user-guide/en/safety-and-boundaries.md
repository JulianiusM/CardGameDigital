# Boundaries and respectful play

## Boundaries are rules

Each player device can exclude question categories, dare types, and operational content
flags. The server combines these limits with profile and intensity before selecting a
card. Other participants learn only that your setup was saved—not which limit belongs
to whom.

Questions apply the active player's limits. Dares and group-wide modes apply the
relevant limits of everyone affected.

## Consent during play

- Anyone may reject a card without explaining why.
- Silence, hesitation, or group pressure is not consent.
- Consent to one card does not automatically cover the next.
- Touch, kissing, alcohol, and adult content require current, voluntary consent from
  everyone involved.
- Stop immediately if anyone becomes uncomfortable.

## A practical safety ritual

Agree on three signals: **green** to continue, **yellow** to slow down and check in,
and **red** to stop the card or session without debate. Pause after intense cards. Host
permissions never override another person's boundaries.

## Adult content

The adult profile requires deliberate confirmation and is only for consenting adults.
Confirmation never disables individual boundaries. If age, consent, or context is
uncertain, choose another profile.
